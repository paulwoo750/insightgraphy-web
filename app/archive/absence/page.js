...
export default function AbsencePage() {
  ...
  const fetchData = async () => {
    ...
    if (configData) {
      ...
      if (wks) curWk = Number(wks)
      if (tWks) totWks = Number(tWks)
      
      setCurrentWeek(curWk)
      setTotalWeeks(totWks)
      setAbsenceForm(prev => ({ ...prev, week: curWk })) // 기본 선택 주차를 현재 주차로
    }

    // 2. 해당 주차 마감 기한 불러오기
    const { data: dlData } = await supabase.from('pr_deadlines').select('deadline_time').eq('week', curWk).eq('category', 'absence').single()
    ...
    // 3. 🌟 모든 학회원의 사유서 제출 내역 다 가져오기 (게시판용)
  ...
  const handleSubmitAbsence = async () => {
    ...
    if (!error) {
      ...
      setAbsenceForm({ week: currentWeek, date: '', type: '결석', time: '', reason: '', proofUrl: '' })
      fetchData()
    } else {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans p-6 md:p-12 pb-32">
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        {/* 🌟 1. 나의 사유서 결재 현황 요약 (Personal Stats) */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-8 items-start">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200">
            ...
            <div className="flex gap-2 mb-6 overflow-x-auto pb-4 no-scrollbar border-b border-slate-100">
              <button 
                ...
                className={`px-4 py-2.5 rounded-xl text-xs font-black shrink-0 transition-all border ${selectedFolder === 'all' ? 'bg-slate-800 text-white shadow-md border-slate-800' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}
              >
                전체 보기 📂
              </button>
              {weeks.map(w => (
                <button 
                  ...
                  className={`px-4 py-2.5 rounded-xl text-xs font-black shrink-0 transition-all border ${selectedFolder === w ? 'bg-purple-600 text-white shadow-md border-purple-600' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}
                >
                  W{w} 사유서
                </button>
              ))}
              ...
              {filteredAbsences.length === 0 ? (
                ...
                return (
                  <div key={abs.id} className={`p-6 rounded-3xl border transition-all relative ${isMine ? 'bg-blue-50/50 border-blue-200' : 'bg-white border-slate-100 hover:border-slate-300'}`}>
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-4 border-b border-slate-100/50 pb-3">
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="bg-purple-100 text-purple-600 px-2 py-0.5 rounded text-[9px] font-black uppercase">
                    ...
                    <p className="text-sm font-bold text-slate-600 whitespace-pre-wrap leading-relaxed">{abs.reason}</p>
                    
                    {abs.proof_url && (
                      <a href={abs.proof_url} target="_blank" rel="noopener noreferrer" className="inline-block mt-3 px-3 py-1.5 bg-slate-100 text-slate-500 rounded-lg text-[10px] font-black hover:bg-slate-200 transition-colors">
                        🔗 첨부된 증빙자료 보기
                      </a>
                    )}
                    ...
                    {abs.status !== '대기' && abs.admin_comment && (
                      <div className="mt-4 p-3 bg-slate-50 rounded-xl border border-slate-100 flex gap-2 items-start">
                        ...
                        <div>
                          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">운영진 결재 코멘트</p>
          ...
          <div className="space-y-6 sticky top-8">
            ...
            {/* 알림 보드 */}
            <div className={`p-6 rounded-[2rem] shadow-sm border transition-all ${isLate ? 'bg-red-50 border-red-200' : 'bg-white border-slate-200'}`}>
              ...
              <h2 className="text-lg font-black text-slate-800 mt-3 flex items-center gap-2 mb-4">
                <span className="bg-blue-100 text-blue-600 w-6 h-6 flex items-center justify-center rounded-full text-xs">W{currentWeek}</span>
                현재 주차 마감일
              </h2>
              <div className="bg-white/60 p-4 rounded-xl border border-white/80">
                {absenceDeadline ? (
                  <p className={`font-black text-lg flex items-center gap-2 ${isLate ? 'text-red-500' : 'text-emerald-600'}`}>
                    {formatTime(absenceDeadline)} 
                    {isLate && <span className="bg-red-500 text-white px-2 py-0.5 rounded text-[9px] uppercase tracking-widest shadow-sm animate-pulse">지각 🚨</span>}
                  </p>
                ) : (
            ...
            <div className="bg-slate-900 p-8 rounded-[2.5rem] shadow-xl border border-slate-800">
              <div className="flex justify-between items-center border-b border-slate-800 pb-4 mb-6">
                <h3 className="text-sm font-black text-white uppercase tracking-widest flex items-center gap-2"><span>✍️</span> 새 사유서 작성</h3>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select 
                    value={absenceForm.week} 
                    onChange={e => setAbsenceForm({...absenceForm, week: Number(e.target.value)})} 
                    className="w-full bg-slate-800 text-white p-3 rounded-xl font-bold text-sm outline-none border border-slate-700 focus:border-blue-500 cursor-pointer"
                  >
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">날짜</label>
                    <input type="date" value={absenceForm.date} onChange={e => setAbsenceForm({...absenceForm, date: e.target.value})} className="w-full bg-slate-800 text-white p-3 rounded-xl font-bold text-sm outline-none border border-slate-700 focus:border-blue-500" />
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">구분</label>
                    <select value={absenceForm.type} onChange={e => setAbsenceForm({...absenceForm, type: e.target.value})} className="w-full bg-slate-800 text-white p-3 rounded-xl font-bold text-sm outline-none border border-slate-700 focus:border-blue-500 cursor-pointer">
                ...
                {absenceForm.type !== '결석' && (
                  <div className="animate-in slide-in-from-top-2">
                    ...
                    <input type="time" value={absenceForm.time} onChange={e => setAbsenceForm({...absenceForm, time: e.target.value})} className="w-full bg-slate-800 text-white p-3 rounded-xl font-bold text-sm outline-none border border-slate-700 focus:border-blue-500" />
                  </div>
                )}

                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">상세 사유</label>
                  <textarea value={absenceForm.reason} onChange={e => setAbsenceForm({...absenceForm, reason: e.target.value})} className="w-full bg-slate-800 text-white p-4 rounded-xl font-bold text-sm outline-none border border-slate-700 focus:border-blue-500 min-h-[100px]" placeholder="구체적인 사유를 작성해 주세요." />
                </div>
                ...
                <div className="bg-slate-800 p-4 rounded-2xl border border-slate-700">
                  ...
                  <div className="flex flex-col gap-2">
                    <a href={GOOGLE_DRIVE_FOLDER_URL} target="_blank" rel="noopener noreferrer" className="bg-slate-700 hover:bg-slate-600 text-slate-300 px-4 py-2.5 rounded-xl font-black text-[10px] transition-all flex items-center justify-center gap-2">
                      <span className="text-base">📁</span> Step 1. 드라이브 업로드
                    </a>
                    <input type="text" value={absenceForm.proofUrl} onChange={e => setAbsenceForm({...absenceForm, proofUrl: e.target.value})} className="w-full bg-slate-900 text-white p-3 rounded-xl font-bold text-xs outline-none border border-slate-600 focus:border-blue-500" placeholder="Step 2. 공유 링크 붙여넣기" />
                  </div>
                </div>

                <button onClick={handleSubmitAbsence} disabled={isSubmitting} className={`w-full py-4 text-white rounded-xl font-black text-sm transition-all shadow-lg mt-2 active:scale-95 ${isLate && absenceForm.week === currentWeek ? 'bg-red-600 hover:bg-red-700' : 'bg-blue-600 hover:bg-blue-700'}`}>
                  {isSubmitting ? '제출 중...' : isLate && absenceForm.week === currentWeek ? '지각 제출하기 🚨' : '게시판에 등록하기 🚀'}
                </button>
              </div>
            </div>
          </div>
