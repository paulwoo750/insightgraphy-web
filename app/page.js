...
import { DEFAULT_LANDING, loadSiteContent } from '@/lib/siteContent'
...
export default function LandingPage() {
  ...
  // 🌟 관리자가 편집한 문구 (pr_config.landing_content)
  const [c, setC] = useState(DEFAULT_LANDING)
  ...
  const fetchLandingData = async () => {
    ...
    // 0. 편집 가능한 문구 불러오기
    setC(await loadSiteContent(supabase, 'landing_content', DEFAULT_LANDING))
    ...
    <div className="min-h-screen bg-white text-[#1a1a1a] font-sans">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-[#0d6b69] via-[#32a4a1] to-[#a8d0cd]">
        ...
        <div className="relative z-10 text-center text-white px-6">
          <p className="text-sm font-bold tracking-[0.5em] mb-4 opacity-80 uppercase text-[#a8d0cd]">{c.hero.eyebrow}</p>
          <h1 className="text-6xl md:text-8xl font-black tracking-tighter uppercase mb-6 leading-none drop-shadow-lg">{c.hero.titleLine1}<br/>{c.hero.titleLine2}</h1>
          <p className="text-lg md:text-xl font-medium max-w-2xl mx-auto leading-relaxed break-keep opacity-95 drop-shadow-md whitespace-pre-line">
            {c.hero.desc}
          </p>
          <div className="mt-12 flex flex-col sm:flex-row justify-center items-center gap-4">
            <Link href="/about" className="px-10 h-[56px] bg-white text-[#1a1a1a] font-black uppercase text-[13px] tracking-widest rounded-full hover:bg-[#a8d0cd] transition-all shadow-lg flex items-center justify-center min-w-[180px]">{c.hero.btnPrimary}</Link>
            <Link href="/join" className="px-10 h-[56px] border-2 border-white text-white font-black uppercase text-[13px] tracking-widest rounded-full hover:bg-white hover:text-[#1a1a1a] transition-all shadow-lg flex items-center justify-center min-w-[180px]">{c.hero.btnSecondary}</Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-8 bg-white max-w-7xl mx-auto border-b border-slate-50">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-black uppercase tracking-tighter text-slate-800">{c.activities.title}</h2>
          <div className="w-12 h-1 bg-[#32a4a1] mx-auto mt-4 rounded-full" />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 text-center">
          {c.activities.items.map((it, i) => (
            <Link key={i} href={it.href || '/activities'} className="space-y-4 p-8 rounded-3xl hover:shadow-xl transition-all hover:bg-slate-50 flex flex-col items-center group cursor-pointer border border-transparent hover:border-slate-100">
              <span className="text-5xl mb-2 text-[#32a4a1] group-hover:scale-110 transition-transform">{it.icon}</span>
              <h3 className="text-xl font-black uppercase">{it.title}</h3>
              <p className="text-sm leading-relaxed text-slate-600 break-keep">{it.desc}</p>
              <span className="text-[10px] font-black uppercase text-[#32a4a1] opacity-0 group-hover:opacity-100 transition-opacity mt-2">{c.activities.cardLink}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="py-24 px-6 bg-[#1a1a1a] text-white">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div>
              <h2 className="text-4xl font-black uppercase tracking-tighter text-white">{c.showcase.title}</h2>
              <div className="w-12 h-1 bg-[#a8d0cd] mt-4 rounded-full" />
              <p className="text-white/60 font-medium mt-4">{c.showcase.desc}</p>
            </div>
          ...
          {loading ? (
            ...
            <div className="text-center text-white/30 py-10 font-bold border border-white/10 rounded-3xl">{c.showcase.empty}</div>
          ) : (
      ...
      <section className="py-28 px-6 bg-slate-50">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-16 space-y-4">
            <h2 className="text-4xl md:text-5xl font-black uppercase tracking-tighter text-[#0d6b69]">{c.sessions.title}</h2>
            <div className="flex items-center justify-center gap-2 text-[#32a4a1] opacity-70">
              <span className="text-xl">“</span>
              <p className="text-xs md:text-sm font-bold italic tracking-wide">{c.sessions.quote}</p>
              <span className="text-xl">”</span>
            </div>
          </div>
      ...
      <footer className="py-12 border-t border-slate-100 text-center bg-[#1a1a1a] text-white">
        ...
        <p className="text-[10px] font-black text-[#a8d0cd] uppercase tracking-[0.5em]">{c.footer.text}</p>
      </footer>
    </div>
  )
}